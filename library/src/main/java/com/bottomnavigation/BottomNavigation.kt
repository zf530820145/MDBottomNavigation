...
import com.zf.bottomnavigation.R

import kotlin.math.abs

/**
 *  Bottom Navigation Cell class.
 *
 * @author  zhangfan
 * @version 1.1.0
 * @since   2020-1-25
...
internal typealias IBottomNavigationListener = (model: MDBottomNavigation.Model) -> Unit

@Suppress("MemberVisibilityCanBePrivate")
class MDBottomNavigation : FrameLayout {
    ...
    var backgroundBottomColor = Color.parseColor("#0c2577")
        set(value) {
            field = value
            updateAllIfAllowDraw()
        }
    var circleColor = Color.parseColor("#0f66b5")
        set(value) {
            field = value
            updateAllIfAllowDraw()
        }
    ...
    private fun setAttributeFromXml(context: Context, attrs: AttributeSet) {
        val a = context.theme.obtainStyledAttributes(
            attrs,
            R.styleable.MDBottomNavigation, 0, 0
        )
        try {
            a.apply {
                defaultIconColor = getColor(
                    R.styleable.MDBottomNavigation_mbn_defaultIconColor,
                    defaultIconColor
                )
                selectedIconColor = getColor(
                    R.styleable.MDBottomNavigation_mbn_selectedIconColor,
                    selectedIconColor
                )
                backgroundBottomColor = getColor(
                    R.styleable.MDBottomNavigation_mbn_backgroundBottomColor,
                    backgroundBottomColor
                )
                circleColor =
                    getColor(R.styleable.MDBottomNavigation_mbn_circleColor, circleColor)
                countTextColor =
                    getColor(R.styleable.MDBottomNavigation_mbn_countTextColor, countTextColor)
                countBackgroundColor = getColor(
                    R.styleable.MDBottomNavigation_mbn_countBackgroundColor,
                    countBackgroundColor
                )
                rippleColor =
                    getColor(R.styleable.MDBottomNavigation_mbn_rippleColor, rippleColor)
                shadowColor =
                    getColor(R.styleable.MDBottomNavigation_mbn_shadowColor, shadowColor)

                val typeface = getString(R.styleable.MDBottomNavigation_mbn_countTypeface)
                if (typeface != null && typeface.isNotEmpty())
                    countTypeface = Typeface.createFromAsset(context.assets, typeface)

                hasAnimation =
                    getBoolean(R.styleable.MDBottomNavigation_mbn_hasAnimation, hasAnimation)
            }
        } finally {
            a.recycle()
        }
    ...
    private fun initializeViews() {
        ...
        bezierView.apply {
            ...
            shadowColor = this@MDBottomNavigation.shadowColor
        }
    ...
    fun add(model: Model) {
        ...
        cell.apply {
            ...
            title=model.title
            count = model.count
            defaultIconColor = this@MDBottomNavigation.defaultIconColor
            selectedIconColor = this@MDBottomNavigation.selectedIconColor
            circleColor = this@MDBottomNavigation.circleColor
            countTextColor = this@MDBottomNavigation.countTextColor
            countBackgroundColor = this@MDBottomNavigation.countBackgroundColor
            countTypeface = this@MDBottomNavigation.countTypeface
            rippleColor = this@MDBottomNavigation.rippleColor
            onClickListener = {
    ...
    fun clearCount(id: Int) {
        val model = getModelById(id) ?: return
        val pos = getModelPosition(id)
        model.count = BottomNavigationCell.EMPTY_VALUE
        cells[pos].count = BottomNavigationCell.EMPTY_VALUE
    }
